...
public interface GroubaOrderMapper {
	...
	@Select(value = "<script>select order_trace,ref_groub_trace,ref_grouba_trace,order_expired_time,order_status,ref_user_wx_unionid,grouba_size,leader,ref_user_img,join_succeed_time,ref_user_wx_openid,goods_name,goods_img,goods_price,grouba_discount_amount,grouba_isnew"
	 ...
	 * @return
	 */
	@Select(value = "<script>select order_trace,ref_groub_trace,ref_grouba_trace,order_expired_time,order_status,ref_user_wx_unionid,leader,ref_user_img,join_succeed_time,ref_user_wx_unionid,grouba_size,goods_name,goods_img,goods_price,grouba_discount_amount,grouba_isnew"
	...
	@Insert(value = "<script>INSERT INTO grouba_order"
			...
			+ ",ref_user_wx_openid,grouba_size,goods_name,goods_img,goods_price,grouba_discount_amount,grouba_isnew) "
			+ " VALUES (#{orderTrace},#{refGroubTrace},#{refGroubaTrace},#{orderExpiredTime},#{refUserWxUnionid},#{leader},#{refUserImg},#{clientIp}"
			+ ",#{refUserWxOpenid},#{groubaSize},#{goodsName},#{goodsImg},#{goodsPrice},#{groubaDiscountAmount},#{groubaIsnew})</script>")
	...
	@Select(value = "<script>select order_trace,ref_groub_trace,ref_grouba_trace,order_expired_time,order_status,ref_user_wx_unionid,grouba_size,leader,ref_user_img,join_succeed_time,goods_name,goods_img,goods_price,grouba_discount_amount,grouba_isnew"
			+ " from grouba_order" + "<where>" + "<if test=\"orderTrace != null and orderTrace != '' \">"
			+ " and order_trace = #{orderTrace}" + "</if>"
			+ "</where></script>")
